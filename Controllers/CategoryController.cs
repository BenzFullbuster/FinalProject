...
{
    public class CategoryController(ConnectDb db, ILogger<CategoryController> logger) : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, string currentFilter, string sortOrder, int? pageSize, int currentPage = 1)
        {
            try
            {
                ...
                var query = (from x in _db.Category
                ...
                if (!string.IsNullOrEmpty(searchString))
                {
                    ...
                    query = query.Where(x => x.CategoryName.ToLower().Contains(searchString.ToLower())
                        || x.Description!.ToLower().Contains(searchString.ToLower()));
                }

                query = sortOrder switch
                {
                    "name" => query.OrderBy(u => u.CategoryName),
                    "name_desc" => query.OrderByDescending(u => u.CategoryName),
                    "description" => query.OrderBy(u => u.Description),
                    "description_desc" => query.OrderByDescending(u => u.Description),
                    _ => query.OrderBy(u => u.CategoryName),
                };
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            ...
            try
            {
                var item = await (from x in _db.Category
        ...
        public async Task<IActionResult> Create(CategoryCreateViewModel model)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (await NameIsExists(Guid.Empty, model.CategoryName))
                    {
                        ModelState.AddModelError("CategoryName", "ชื่อนี้มีอยู่แล้ว");
                        return View(model);
                    }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, CategoryEditViewModel model)
        {
            if (id == Guid.Empty)
            {
                TempData["Warning"] = "ไม่มีข้อมูลในตาราง";
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (await NameIsExists(model.CategoryId, model.CategoryName))
                    {
                        ModelState.AddModelError("CategoryName", "ชื่อนี้มีอยู่แล้ว");
                        return View(model);
                    }

                    var oldItem = await _db.Category.FirstOrDefaultAsync(x => x.CategoryId == id);
                    ...
                    if (oldItem != null)
                    {
                        ...
                        if (oldItem.CategoryName != model.CategoryName)
                        {
                            oldItem.CategoryName = model.CategoryName;
                            isUpdated = true;
                        }
        ...
        public async Task<IActionResult> DeleteConfirmed(Guid id, byte[] rowVersion)
        {
            ...
            try
            {
                var item = await _db.Category.FirstOrDefaultAsync(x => x.CategoryId == id);
                ...
                if (item != null)
                {
                    ...
                    if (await CheckCategoryInProduct(id))
                    {
                        TempData["Warning"] = $"ไม่สามารถลบ {item.CategoryName} ได้เนื่องจากมีการอ้างอิงอยู่";
        ...
        public async Task<IActionResult> ExportCsv()
        {
            try
            {
                var items = await (from x in _db.Category
                ...
                builder.AppendLine("CategoryId,CategoryName,Description,CreatedDate,ModifiedDate");

                foreach (var item in items)
                {
                    builder.AppendLine($"{item.CategoryId}, {item.CategoryName}, {item.Description}, " +
                ...
                var fileName = "รายการหมวดสินค้า.csv";
        ...
        public async Task<IActionResult> ExportExcel()
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("หมวดสินค้า");

                var fontName = "Angsana New"; //กำหนด Font

                var headerRange = worksheet.Range("A1:E1");
                headerRange.Merge().Value = "รายการหมวดสินค้า"; //ผลาสเซลล์
                ...
                worksheet.Cell("A2").Value = "รหัสหมวดสินค้า";
                worksheet.Cell("B2").Value = "ชื่อหมวดสินค้า";
                ...
                foreach (var item in items)
                {
                    worksheet.Cell($"A{rowNumber}").Value = item.CategoryId.ToString();
                    worksheet.Cell($"B{rowNumber}").Value = item.CategoryName.ToString();
                ...
                var fileName = "รายการหมวดสินค้า.xlsx";
            ...
            try
            {
                ...
                var document = Document.Create(container =>
                {
                    container.Page(page =>
                    {
                        ...
                        //{
                        //    row.RelativeItem().AlignCenter().Text("รายงานหมวดสินค้า")
                        //        .SemiBold().FontSize(30).FontColor(Colors.Blue.Medium);
                        //});

                        page.Header().ShowOnce().Text("รายงานหมวดสินค้า").AlignCenter()
                        .SemiBold().FontSize(30).FontColor(Colors.Blue.Medium);
                ...
                var fileName = "รายงานหมวดสินค้า.pdf";
        ...
        private async Task<bool> NameIsExists(Guid categoryId, string categoryName)
        {
            return await _db.Category.AsNoTracking().AnyAsync(x => x.CategoryId != categoryId && x.CategoryName == categoryName);
        }

        private async Task<bool> CheckCategoryInProduct(Guid categoryId)
        {
            return await _db.Product.AsNoTracking().AnyAsync(x => x.CategoryId == categoryId);
        }
    }
}
