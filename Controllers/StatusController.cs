...
{
    public class StatusController(ConnectDb db, ILogger<StatusController> logger) : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, string currentFilter, string sortOrder, int? pageSize, int currentPage = 1)
        {
            try
            {
                ...
                var query = (from x in _db.Status
                ...
                if (!string.IsNullOrEmpty(searchString))
                {
                    ...
                    query = query.Where(x => x.StatusName.ToLower().Contains(searchString.ToLower())
                        || x.Description!.ToLower().Contains(searchString.ToLower()));
                }

                query = sortOrder switch
                {
                    "name" => query.OrderBy(u => u.StatusName),
                    "name_desc" => query.OrderByDescending(u => u.StatusName),
                    "description" => query.OrderBy(u => u.Description),
                    "description_desc" => query.OrderByDescending(u => u.Description),
                    _ => query.OrderBy(u => u.StatusName),
                };
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            ...
            try
            {
                var item = await (from x in _db.Status
                                  where x.StatusId == id
                                  select new StatusViewModel
                                  {
                                      StatusId = x.StatusId,
        ...
        public async Task<IActionResult> Create(StatusCreateViewModel model)
        {
            ...
            try
            {
                ...
                {
                    model.StatusName = model.StatusName.Trim();

                    if (await NameIsExists(Guid.Empty, model.StatusName))
                    {
                        ModelState.AddModelError("StatusName", "ชื่อนี้มีอยู่แล้ว");
                        return View(model);
                    }
                    ...
                    await _db.Status.AddAsync(item);
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            try
            {
                var item = await (from x in _db.Status
                                  where x.StatusId == id
                                  select new StatusEditViewModel
                                  {
                                      StatusId = x.StatusId,
                                      StatusName = x.StatusName,
                                      Description = x.Description,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, StatusEditViewModel model)
        {
            if (id == Guid.Empty)
            {
                TempData["Warning"] = "ไม่มีข้อมูลในตาราง";
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (await NameIsExists(model.StatusId, model.StatusName))
                    {
                        ModelState.AddModelError("StatusName", "ชื่อนี้มีอยู่แล้ว");
                        return View(model);
                    }

                    var oldItem = await _db.Status.FirstOrDefaultAsync(x => x.StatusId == id);
                    ...
                    if (oldItem != null)
                    {
                        ...
                        if (oldItem.StatusName != model.StatusName)
                        {
                            oldItem.StatusName = model.StatusName;
                            isUpdated = true;
                        }
        ...
        public async Task<IActionResult> DeleteConfirmed(Guid id, byte[] rowVersion)
        {
            ...
            try
            {
                var item = await _db.Status.FirstOrDefaultAsync(x => x.StatusId == id);
                ...
                if (await CheckStatusInPurchaseOrder(id))
                {
                    TempData["Warning"] = $"ไม่สามารถลบ {item.StatusName} ได้เนื่องจากมีการอ้างอิงอยู่";
        ...
        public async Task<IActionResult> ExportCsv()
        {
            try
            {
                var items = await (from x in _db.Status
                ...
                builder.AppendLine("StatusId,StatusName,Description,CreatedDate,ModifiedDate");

                foreach (var item in items)
                {
                    builder.AppendLine($"{item.StatusId}, {item.StatusName}, {item.Description}, " +
        ...
        public async Task<IActionResult> ExportExcel()
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("สถานะ");

                var fontName = "Angsana New"; //กำหนด Font

                var headerRange = worksheet.Range("A1:E1");
                headerRange.Merge().Value = "รายการสถานะ"; //ผลาสเซลล์
                ...
                worksheet.Cell("A2").Value = "รหัสสถานะ";
                worksheet.Cell("B2").Value = "ชื่อสถานะ";
                ...
                foreach (var item in items)
                {
                    worksheet.Cell($"A{rowNumber}").Value = item.StatusId.ToString();
                    worksheet.Cell($"B{rowNumber}").Value = item.StatusName.ToString();
            ...
            try
            {
                ...
                var document = Document.Create(container =>
                {
                    container.Page(page =>
                    {
                        ...
                        page.Header().ShowOnce().Text("รายงานสถานะ").AlignCenter()
                        ...
                        page.Content().Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().Element(thead).Text("สถานะ").SemiBold();
                            ...
                            for (int i = 0; i < items.Count; i++)
                            {
                                ...
                                table.Cell().Element(tbody).Text(item.StatusName);
        ...
        private async Task<bool> NameIsExists(Guid statusId, string statusName)
        {
            return await _db.Status.AsNoTracking().AnyAsync(x => x.StatusId != statusId && x.StatusName == statusName);
        }

        private async Task<bool> CheckStatusInPurchaseOrder(Guid statusId)
        {
            return await _db.PurchaseOrder.AsNoTracking().AnyAsync(x => x.StatusId == statusId);
        }
    }
}
