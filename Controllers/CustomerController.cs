...
{
    public class CustomerController(ConnectDb db, ILogger<CustomerController> logger) : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchBy, string searchString, string currentFilter, string sortOrder, int? pageSize, int currentPage = 1)
        {
            try
            {
                ...
                var query = (from s in _db.Customer
                ...
                if (!string.IsNullOrEmpty(searchString))
                {
                    ...
                    {
                        "CustomerName" => query = query.Where(x => x.CustomerName.ToLower().Contains(searchString.ToLower())),
                ...
                query = sortOrder switch
                {
                    ...
                    "name_desc" => query.OrderByDescending(x => x.CustomerName),
                    ...
                    _ => query.OrderBy(x => x.CustomerName)
                };
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            ...
            try
            {
                var item = await (from s in _db.Customer
                                  join p in _db.Province on s.ProvinceId equals p.ProvinceId into pJoin
                                  from p in pJoin.DefaultIfEmpty()
                                  join d in _db.District on s.DistrictId equals d.DistrictId into dJoin
                                  from d in dJoin.DefaultIfEmpty()
        ...
        public async Task<IActionResult> Create(CustomerCreateViewModel model)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (await NameIsExists(Guid.Empty, model.CustomerName))
                    {
                        ModelState.AddModelError("CustomerName", "ชื่อนี้มีอยู่แล้ว");
                        return View(model);
                    }
                    ...
                    await _db.Customer.AddAsync(item);
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            try
            {
                var item = await (from s in _db.Customer
                                  ...
                                  from sd in sdJoin.DefaultIfEmpty()
                                  where s.CustomerId == id
                                  select new CustomerEditViewModel
                                  {
                                      CustomerId = s.CustomerId,
                                      CustomerName = s.CustomerName,
                                      Address = s.Address,
        ...
        public async Task<IActionResult> Edit(Guid id, CustomerEditViewModel model)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (await NameIsExists(model.CustomerId, model.CustomerName))
                    {
                        ModelState.AddModelError("CustomerName", "ชื่อนี้มีอยู่แล้ว");
                        return View(model);
                    }

                    var oldItem = await _db.Customer.FirstOrDefaultAsync(x => x.CustomerId == id);
                    ...
                    if (oldItem != null)
                    {
                        ...
                        if (oldItem.CustomerName != model.CustomerName)
                        {
        ...
        public async Task<ActionResult> DeleteConfirmed(Guid id, byte[] rowVersion)
        {
            ...
            try
            {
                var item = await _db.Customer.Where(x => x.CustomerId == id).FirstOrDefaultAsync();
                ...
                if (item != null)
                {
                    ...
                    if (await CheckCustomerRelatedToOperations(id))
                    {
                        TempData["Warning"] = $"ไม่สามารถลบ {item.CustomerName} ได้เนื่องจากมีการอ้างอิงอยู่";
        ...
        public async Task<IActionResult> ExportCsv()
        {
            try
            {
                var items = await (from e in _db.Customer
        ...
        public async Task<IActionResult> ExportExcel()
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("ลูกค้า");

                var fontName = "Angsana New"; //กำหนด Font

                var headerRange = worksheet.Range("A1:M1");
                headerRange.Merge().Value = "รายการลูกค้า"; //ผลาสเซลล์
                ...
                worksheet.Cell("A2").Value = "รหัสลูกค้า";
                worksheet.Cell("B2").Value = "ชื่อลูกค้า";
                ...
                foreach (var item in items)
                {
                    worksheet.Cell($"A{rowNumber}").Value = item.CustomerId.ToString();
                    worksheet.Cell($"B{rowNumber}").Value = item.CustomerName.ToString();
                ...
                var fileName = "รายการลูกค้า.xlsx";
            ...
            try
            {
                ...
                var document = Document.Create(container =>
                {
                    container.Page(page =>
                    {
                        ...
                        page.Header().ShowOnce().Text("รายงานลูกค้า").AlignCenter()
                        ...
                        page.Content().Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().Element(thead).Text("ลูกค้า").SemiBold();
                            ...
                            for (int i = 0; i < items.Count; i++)
                            {
                                ...
                                table.Cell().Element(tbody).Text(item.CustomerName);
        ...
        private async Task<bool> NameIsExists(Guid customerId, string customerName)
        {
            return await _db.Customer.AsNoTracking().AnyAsync(x => x.CustomerId != customerId && x.CustomerName == customerName);
        }

        private async Task<bool> CheckCustomerRelatedToOperations(Guid customerId)
        {
            return await _db.DeliveryGoods.AsNoTracking().AnyAsync(x => x.CustomerId == customerId)
                || await _db.GoodsReturnFromCustomer.AsNoTracking().AnyAsync(x => x.CustomerId == customerId);
        }
    }
}
