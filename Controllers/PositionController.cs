...
{
    public class PositionController(ConnectDb db, ILogger<PositionController> logger) : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, string currentFilter, string sortOrder, int? pageSize, int currentPage = 1)
        {
            try
            {
                ...
                var query = (from x in _db.Position
                ...
                if (!string.IsNullOrEmpty(searchString))
                {
                    ...
                    query = query.Where(x => x.PositionName.ToLower().Contains(searchString.ToLower())
                        || x.Description!.ToLower().Contains(searchString.ToLower()));
                }

                query = sortOrder switch
                {
                    "name" => query.OrderBy(u => u.PositionName),
                    "name_desc" => query.OrderByDescending(u => u.PositionName),
                    "description" => query.OrderBy(u => u.Description),
                    "description_desc" => query.OrderByDescending(u => u.Description),
                    _ => query.OrderBy(u => u.PositionName),
                };
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            ...
            try
            {
                var item = await (from x in _db.Position
                                  where x.PositionId == id
                                  select new PositionViewModel
                                  {
                                      PositionId = x.PositionId,
                                      PositionName = x.PositionName,
        ...
        public async Task<IActionResult> Create(PositionCreateViewModel model)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (await NameIsExists(Guid.Empty, model.PositionName))
                    {
                        ModelState.AddModelError("PositionName", "ชื่อนี้มีอยู่แล้ว");
                        return View(model);
                    }
                    ...
                    await _db.Position.AddAsync(item);
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            try
            {
                var item = await (from x in _db.Position
                                  where x.PositionId == id
                                  select new PositionEditViewModel
                                  {
                                      PositionId = x.PositionId,
                                      PositionName = x.PositionName,
                                      Description = x.Description,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, PositionEditViewModel model)
        {
            if (id == Guid.Empty)
            {
                TempData["Warning"] = "ไม่มีข้อมูลในตาราง";
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (await NameIsExists(model.PositionId, model.PositionName))
                    {
                        ModelState.AddModelError("PositionName", "ชื่อนี้มีอยู่แล้ว");
                        return View(model);
                    }

                    var oldItem = await _db.Position.FirstOrDefaultAsync(x => x.PositionId == id);
                    ...
                    if (oldItem != null)
                    {
                        ...
                        if (oldItem.PositionName != model.PositionName)
                        {
                            oldItem.PositionName = model.PositionName;
                            isUpdated = true;
                        }
        ...
        public async Task<IActionResult> DeleteConfirmed(Guid id, byte[] rowVersion)
        {
            ...
            try
            {
                var item = await _db.Position.FirstOrDefaultAsync(x => x.PositionId == id);
                ...
                if (item != null)
                {
                    ...
                    if (await CheckPositionInEmployee(id))
                    {
                        TempData["Warning"] = $"ไม่สามารถลบ {item.PositionName} ได้เนื่องจากมีการอ้างอิงอยู่";
        ...
        public async Task<IActionResult> ExportCsv()
        {
            try
            {
                var items = await (from x in _db.Position
                                   orderby x.PositionName
                                   select new PositionViewModel
                                   {
                                       PositionId = x.PositionId,
                                       PositionName = x.PositionName,
                ...
                builder.AppendLine("PositionId,PositionName,Description,CreatedDate,ModifiedDate");

                foreach (var item in items)
                {
                    builder.AppendLine($"{item.PositionId}, {item.PositionName}, {item.Description}, " +
                ...
                var fileName = "รายการตำแหน่ง.csv";
        ...
        public async Task<IActionResult> ExportExcel()
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("ตำแหน่ง");

                var fontName = "Angsana New"; //กำหนด Font

                var headerRange = worksheet.Range("A1:E1");
                headerRange.Merge().Value = "รายการตำแหน่ง"; //ผลาสเซลล์
                ...
                worksheet.Cell("A2").Value = "รหัสตำแหน่ง";
                worksheet.Cell("B2").Value = "ชื่อตำแหน่ง";
                ...
                foreach (var item in items)
                {
                    worksheet.Cell($"A{rowNumber}").Value = item.PositionId.ToString();
                    worksheet.Cell($"B{rowNumber}").Value = item.PositionName.ToString();
                ...
                var fileName = "รายการตำแหน่ง.xlsx";
            ...
            try
            {
                ...
                var document = Document.Create(container =>
                {
                    container.Page(page =>
                    {
                        ...
                        //{
                        //    row.RelativeItem().AlignCenter().Text("รายงานตำแหน่ง")
                        //        .SemiBold().FontSize(30).FontColor(Colors.Blue.Medium);
                        //});

                        page.Header().ShowOnce().Text("รายงานตำแหน่ง").AlignCenter()
                        .SemiBold().FontSize(30).FontColor(Colors.Blue.Medium);
                        ...
                        page.Content().Table(table =>
                        {
                            ...
                            for (int i = 0; i < items.Count; i++)
                            {
                                ...
                                table.Cell().Element(tbody).Text(item.PositionName);
                ...
                var fileName = "รายงานตำแหน่ง.pdf";
        ...
        private async Task<bool> NameIsExists(Guid positionId, string positionName)
        {
            return await _db.Position.AsNoTracking().AnyAsync(x => x.PositionId != positionId && x.PositionName == positionName);
        }

        private async Task<bool> CheckPositionInEmployee(Guid positionId)
        {
            return await _db.Employee.AsNoTracking().AnyAsync(x => x.PositionId == positionId);
        }
    }
}
